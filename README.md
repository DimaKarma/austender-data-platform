...
The diagram above is drawn by hand. The one below is not — it is generated from
dbt's compiled `manifest.json` by `austender_project/scripts/generate_lineage.py`,
so every arrow is a real `ref()` and it cannot show a dependency the code does not
have. Regenerate with `dbt compile && python scripts/generate_lineage.py`; the
same script run with `--check` fails if the block is stale. The interactive
version, with column-level descriptions, is in the
[published dbt docs](https://dimakarma.github.io/austender-data-platform/).

<!-- BEGIN LINEAGE (generated by austender_project/scripts/generate_lineage.py) -->
```mermaid
flowchart LR
    subgraph SRC["BRONZE — raw, all STRING"]
        raw_abr_entity([raw_abr_entity])
        raw_contract_data([raw_contract_data])
    end
    subgraph STG["SILVER — staging"]
        stg_abr_entity[stg_abr_entity]
        stg_abr_names[stg_abr_names]
        stg_contracts[stg_contracts]
    end
    subgraph SLV["SILVER — cleansed"]
        int_abr_name_lookup[int_abr_name_lookup]
        slv_contracts[slv_contracts]
    end
    subgraph GLD["GOLD — star schema"]
        ...
        fct_contracts[fct_contracts]
    end
    subgraph MRT["MART — consumption"]
        rpt_agency_spend[rpt_agency_spend]
        rpt_bi_dim_agency[rpt_bi_dim_agency]
        rpt_bi_dim_category[rpt_bi_dim_category]
        rpt_bi_dim_date[rpt_bi_dim_date]
        rpt_bi_dim_supplier[rpt_bi_dim_supplier]
        rpt_category_spend[rpt_category_spend]
        rpt_contracts[rpt_contracts]
        rpt_supplier_history[rpt_supplier_history]
        rpt_supplier_spend[rpt_supplier_spend]
    end
    subgraph OTH["Snapshots &amp; seeds"]
        known_non_suppliers[(known_non_suppliers)]
        supplier_history[(supplier_history)]
    end
    dim_agency --> rpt_bi_dim_agency
    dim_agency --> rpt_contracts
    dim_category --> rpt_bi_dim_category
    dim_category --> rpt_contracts
    dim_date --> rpt_bi_dim_date
    dim_date --> rpt_contracts
    dim_supplier --> rpt_bi_dim_supplier
    dim_supplier --> rpt_contracts
    dim_supplier --> supplier_history
    fct_contracts --> rpt_contracts
    int_abr_name_lookup --> dim_supplier
    known_non_suppliers --> rpt_contracts
    raw_abr_entity --> stg_abr_entity
    raw_abr_entity --> stg_abr_names
    raw_contract_data --> stg_contracts
    rpt_contracts --> rpt_agency_spend
    rpt_contracts --> rpt_category_spend
    rpt_contracts --> rpt_supplier_spend
    slv_contracts --> dim_agency
    slv_contracts --> dim_category
    slv_contracts --> dim_supplier
    slv_contracts --> fct_contracts
    slv_contracts --> int_abr_name_lookup
    stg_abr_entity --> dim_supplier
    stg_abr_names --> int_abr_name_lookup
    stg_contracts --> slv_contracts
    supplier_history --> rpt_supplier_history
```

*23 nodes, 27 edges — generated from `target/manifest.json`, so every arrow is a real `ref()`.*
<!-- END LINEAGE -->
...
│   ├── scripts/                     # generate_lineage.py (README DAG from manifest)
