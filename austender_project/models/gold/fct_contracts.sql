...
with versions as (
    ...
    {% if is_incremental() %}
        ...
        where {{ base_contract_id('s.contract_id') }} in (
            select distinct {{ base_contract_id('sc.contract_id') }}
            from {{ ref('slv_contracts') }} as sc
            where sc.loaded_at > (select max(f.loaded_at) from {{ this }} as f)
        )
    {% endif %}
),
